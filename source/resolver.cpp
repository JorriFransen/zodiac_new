  ...
  case AST_Expression_Kind::DOT: {
    ...
    if (parent_decl->kind == AST_Declaration_Kind::IMPORT) {
      ...
      if (parent_type->kind == AST_Type_Kind::ARRAY) {
        ...
        assert(expression->dot.kind == AST_Dot_Expression_Kind::UNKNOWN ||
               expression->dot.kind ==
                   AST_Dot_Expression_Kind::AGGREGATE_OFFSET);
    ...
    if (name == Builtin::atom_exit) {
      ...
      for (int64_t i = 0; i < args.count; i++) {
        ...
        if (args[i]->type->kind == AST_Type_Kind::POINTER) {
          args[i] = ast_cast_expression_new(
...
void convert_condition_to_bool(Resolver *resolver, AST_Expression **expr_ptr) {
  ...
  AST_Expression *null_lit = ast_null_literal_expression_new(
      resolver->allocator, cond_expr->scope, cond_expr->begin_file_pos,
      cond_expr->end_file_pos);

  null_lit->infer_type_from = cond_expr;

  *expr_ptr = ast_binary_expression_new(
      resolver->allocator, BINOP_NEQ, cond_expr, null_lit, cond_expr->scope,
      cond_expr->begin_file_pos, cond_expr->end_file_pos);

  cond_expr = *expr_ptr;

  if (!try_resolve_expression(resolver, null_lit))
    assert(false);
  if (!try_resolve_expression(resolver, cond_expr))
    assert(false);
